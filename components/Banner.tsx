...
import { X, Shield, Clock, Star, Truck } from 'lucide-react';
import { cn, getString } from '../lib/utils';

interface BannerProps {
  message?: string;
  type?: 'discount' | 'shipping' | 'announcement' | 'amazon-partner';
  dismissible?: boolean;
  urgency?: boolean;
  className?: string;
}
...
const Banner: React.FC<BannerProps> = ({
  message = getString('promotions.trustBanner'),
  type = 'amazon-partner',
  dismissible = true,
  urgency = false,
  className
  ...
  const getBannerStyles = () => {
    switch (type) {
      ...
      case 'amazon-partner':
        return 'bg-gradient-to-r from-orange-500 via-orange-600 to-orange-700 text-white shadow-lg';
      default:
  ...
  return (
    <div className={cn(
      'relative py-4 px-4 text-center text-sm font-semibold',
      getBannerStyles(),
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-center gap-3">
        ...
        <div className="flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full">
          ...
          <span className="text-xs font-bold">{getString('promotions.amazonPartner')}</span>
        </div>
